...
% Filtering 30_sleep
%%
for k = size(Vecto_FR3,1):-1:1
    for l = size(Vecto_FR3,2):-1:2
    if (Vecto_FR3(k,l)) >= 800;
        %Vecto_AL3(k,:)=[];
        Vecto_FR3(k,:)=[];
        %Vecto_FL3(k,:)=[];
        %Vecto_AR3(k,:)=[];
...
for k = size(Vecto_FR3w,1):-1:1
    for l = size(Vecto_FR3w,2):-1:2
    if (Vecto_FR3w(k,l)) >= 800;
       % Vecto_AL3w(k,:)=[];
        Vecto_FR3w(k,:)=[];
        Vecto_FL3w(k,:)=[];
        %RVecto_AR3w(k,:)=[];

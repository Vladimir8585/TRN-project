...
thr = 5 ; % threshold (3.5 sometimes based on the visual examination)
...
for i = 1:Nr
    mills(i,:) = FL(1,((i-1)*N_win+1) :(i*N_win)); %% Frontal Right!!
...
for i = 1:Nr
    vills(i,:) = FL(1,((i-1)*N_win+1) :(i*N_win)); %% Frontal right
...
Wake_tr = FL(Wake_ind);
Wake_tr_On = FL(Wake_ind_On);
Wake_tr_Off = FL(Wake_ind_Off);
...
Sleep_tr = FL(Sleep_ind);
Sleep_tr_On = FL(Sleep_ind_On);
Sleep_tr_Off = FL(Sleep_ind_Off);
...
axis([0,tx,0.5,1.5]);
subplot(7,1,6);
imagesc(Gt,Gf,10*log10(FL38'),[0 maxDbG])
axis xy; xlabel('Time(s)'); ylabel('Freq(Hz)')
title('Frontal EEG')
%colorbar;
subplot(7,1,7);
imagesc(t,f,10*log10(FL03'),[0 maxDb])
...
REM_tr = FL(REM_ind);
REM_tr_On = FL(REM_ind_On);
REM_tr_Off = FL(REM_ind_Off);
...
SWS_tr = FL(SWS_ind);
SWS_tr_On = FL(SWS_ind_On);
SWS_tr_Off = FL(SWS_ind_Off);
...
for a = 1:N3_rows-2
    if Scorr_sig3(a,2) == 1 && (Scorr_sig3(a+1,3))>=59999 && ...
        ...
        Vecto_EMG6(Nk60,:) = EMG(1,(Scorr_sig3(a,1)-60000):(Scorr_sig3(a,1)+119999));
    ...
    elseif Scorr_sig3(a,2) == 1 && (Scorr_sig3(a+1,3))>=29999 && (Scorr_sig3(a+1,3))< 35000 &&...
       ...
       % Vecto_FR3(Nk30,:) = FR(1,(Scorr_sig3(a,1)-30000):(Scorr_sig3(a,1)+59999));
        Vecto_EMG3(Nk30,:) = EMG(1,(Scorr_sig3(a,1)-30000):(Scorr_sig3(a,1)+59999));
    ...
    elseif Scorr_sig3(a,2) == 1 && (Scorr_sig3(a+1,3))>=59999 && ...
       ...
       % Vecto_FR6w(Nk60w,:) = FR(1,(Scorr_sig3(a,1)-60000):(Scorr_sig3(a,1)+119999));
        Vecto_EMG6w(Nk60w,:) = EMG(1,(Scorr_sig3(a,1)-60000):(Scorr_sig3(a,1)+119999));
    ...
    elseif Scorr_sig3(a,2) == 1 && (Scorr_sig3(a+1,3))>=29999 && (Scorr_sig3(a+1,3))< 35000 &&...
       ...
       % Vecto_FR3w(Nk30w,:) = FR(1,(Scorr_sig3(a,1)-30000):(Scorr_sig3(a,1)+59999));
        Vecto_EMG3w(Nk30w,:) = EMG(1,(Scorr_sig3(a,1)-30000):(Scorr_sig3(a,1)+59999));
